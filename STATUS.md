...
- Assistant integration:
  - Reverted to `responses` API with system prompt (env `OPENAI_ASSISTANT_PROMPT` or default Tangmo/Arunee wording) and explicit model (`OPENAI_MODEL` or `gpt-4o-mini`) because `assistant_id` is not accepted by responses endpoint
- Carousel:
